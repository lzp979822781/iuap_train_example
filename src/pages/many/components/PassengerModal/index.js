...
class AddEditPassenger extends Component {
    ...
    async componentWillReceiveProps(nextProps) {
        ...
        // 判断btnFlag和currentIndex与当前值是否有改变，如果有则执行
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                // rowData id和ts存在则需要保存，表明当前是修改或详情状态
                if (rowData && rowData.id) {
                    values.id = rowData.id;
                    values.ts = rowData.ts;
                }
                /* 参照处理 dept: "{"refname":"web","refpk":"0cb34d6a-b3cd-4f3c-ab82-57ad6395b9ee"}"
                但后台保存的只是refpk值 */
                const {dept} = values;
                if (dept) {
                ...
                actions.many.savePassenger(values); //保存主表数据
    ...
    /**
     *
     * @description 底部按钮是否显示处理函数,详情状态下不现实确定取消按钮
